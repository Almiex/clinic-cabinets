...
BASE_COLORS = {
    # --- Красные, оранжевые и желтые (четко разделенные) ---
    'Хирургия': '#FF002B',          # Чистый алый, бескомпромиссный красный
    'Неврология': '#6F1D1B',        # Глубокий шоколадно-бордовый, древесный
    'Онкология': '#FFAA00',         # Золотисто-оранжевый (больше желтизны, как вы просили)
    'Эндокринология': '#FF6B00',    # Чистый насыщенный оранжевый (апельсин)
    'Травматология': '#FFE600',     # Яркий лимонно-yellow (сильный контраст)
    'Перевязочная': '#FFE5A3',       # Нежно-песочный, пастельный желтый
    'Забор биоматериала': '#FFCAD4', # Пастельный зефирно-розовый
    'КВС': '#D81159',               # Глубокий темно-розовый краплак

    # --- Розовые и маджента ---
    'Флебология': '#FF006E',        # Оставлен ваш неоновый кармин/ярко-малиновый
    'Гинекология': '#FAA307',       # Переведена в теплый медово-желтый для ухода от розового КВС/Флебологии

    # --- Зеленые оттенки (новые контрастные) ---
    'Пульмонология': '#00A896',     # Морская волна, глубокий бирюзово-зеленый
    'Физиотерапия': '#38B000',      # Яркий травяной зеленый
    'Лаборатория': '#70E000',       # Светло-зеленый салатовый, неоновый
    'Аллергология-иммунология': '#023E8A', # Уведена в глубокий полуночный синий

    # --- Синие и голубые оттенки (разведены по яркости) ---
    'Урология': '#00296B',          # Очень темный строгий синий
    'ЛОР': '#3A86FF',               # Яркий королевский синий
    'Функц. диагностика': '#00B4D8', # Насыщенный лазурно-голубой
    'Терапия': '#90E0EF',           # Светлый прозрачно-голубой
    'Психология': '#E0FAFF',        # Сверхсветлый аквамарин (почти белый воздух)
    'Стационар': '#0077B6',         # Синий океан

    # --- Фиолетовые и сиреневые (разведены по тону) ---
    'УЗИ': '#D8BBFF',               # Нежная светлая сирень
    'Дерматология': '#3D0066',      # Густая темная слива (очень темный)
    'Колопроктология': '#9D4EDD',   # Насыщенный чистый фиолетовый
    'Рентген': '#240046',           # Чернично-черный (максимальное затемнение)

    # --- Коричневые и земляные ---
    'Гастроэнтерология': '#7A5123', # Спокойный классический коричневый
    'Мануальная терапия': '#DEAB90', # Теплый бежевый, телесный

    # --- Нейтральные и служебные ---
    'Процедурные': '#B5E2FA',       # Мягкий ледяной оттенок
    'Психиатрия': '#4A5568',        # Темный графитово-серый
    'Администрация': '#A0AEC0',     # Средний холодный серый
    'Прочее': '#CBD5E0',            # Светло-серый
    'Пусто': '#EDF2F7',             # Чистый платиновый фон
    'Нет данных': '#F7FAFC'         # Почти белый сигнальный фон
}
