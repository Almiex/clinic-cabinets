...
def parse_excel_new(file_bytes):
    ...
    # ИСПРАВЛЕНИЕ: была кириллическая 'м' вместо латинской 'm'
...
def create_overview_heatmap(df, selected_cabinets, selected_dates, colors):
    ...
    for date_short, cab, spec, period, doctor_initials, hours in zip(
        ...
        if spec in ('Пусто', 'Нет данных'):
            ...
            # 12 часов = 100% непрозрачности. Минимум 0.4, чтобы цвет был виден
            alpha = min(max(hours / 12.0, 0.4), 1.0) if hours else 0.4
